...
const sortMeetingsByDay = (arr) => {
  ...
  let arrI = arr.map( meeting => days.indexOf(meeting.dayOfWeek));
  return arr.sort( (a,b) => arrI[arr.indexOf(a)] - arrI[arr.indexOf(b)])
};
...
const sortSchedule = (arr) => {
  // Solution code here...
  let days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'];
  let startTime = arr.map(time => (days.indexOf(time.dayOfWeek)*10000) + parseInt(time.start.substring(0,2), 10) + (parseInt(time.start.substring(2),10)/60));
  let endTime = arr.map(time => (days.indexOf(time.dayOfWeek)*10000) + parseInt(time.end.substring(0,2), 10) + (parseInt(time.end.substring(2),10)/60));
  return arr.sort( (a,b) => {
    if (startTime[arr.indexOf(a)] > startTime[arr.indexOf(b)]) {
      return 1;
    }
    if (startTime[arr.indexOf(a)] < startTime[arr.indexOf(b)]) {
      return -1;
    }
    if (endTime[arr.indexOf(a)] > endTime[arr.indexOf(b)]) {
      return 1;
    }
    if (endTime[arr.indexOf(a)] < endTime[arr.indexOf(b)]) {
      return -1;
